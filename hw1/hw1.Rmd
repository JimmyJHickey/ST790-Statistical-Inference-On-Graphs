...
nodeinfo <- mutate(nodeinfo, val = val, close=closeness, between=betweenness, pg = pg, 
                   katz = katz, hub = hub, auth = auth)
...
delta_power = function(p, q, n, m, reps)
{
  ...
  for (i in 1:reps)
  {
    ...
    # is this power??
    power_delta = pgumbel(y, 0 , 1) 
    powers[i] = power_delta
  }
...
delta_power(p,q,n, m= 10, n_reps)
delta_power(p,q,n, m= 25, n_reps)
delta_power(p,q,n, m= 50, n_reps)
delta_power(p,q,n, m= 100, n_reps)


### Triangles

tau_power = function(p, q, n, m, reps)
{
  require(igraph)
  powers = rep(0,reps)
  ...
  for (i in 1:reps)
  {
    ...
    powers[i] = pnorm(tau_test, 0, 1)
  }
  
  return(mean(powers))
}

tau_power(p,q,n, m= 10, n_reps)
tau_power(p,q,n, m= 25, n_reps)
tau_power(p,q,n, m= 50, n_reps)
tau_power(p,q,n, m= 100, n_reps)
